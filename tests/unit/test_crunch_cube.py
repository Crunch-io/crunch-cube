...
def test_divide_and_apply_mask(division_fixture):
    denominator, expected_mask = division_fixture
    numerator = np.ones((3, 3), dtype=float)
    result = CrunchCube._divide_and_apply_mask(numerator, denominator)
    if expected_mask is None:
        assert type(result) is not np.ma.core.MaskedArray
    else:
        actual_mask = result.mask
...
def division_fixture(request):
    raw_denominator, mask, expected_mask = request.param
    denominator = np.array(raw_denominator)
    if mask is not None:
        denominator = np.ma.masked_array(denominator, mask)
    if expected_mask is not None:
        expected_mask = np.array(expected_mask)
    return denominator, expected_mask
