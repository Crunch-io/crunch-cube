...
def test_ca_cat_x_items():
    slice_ = FrozenCube(SM.CA_CAT_X_ITEMS).partitions[0]
...
def test_ca_items_x_cat():
    slice_ = FrozenCube(SM.CA_ITEMS_X_CAT).partitions[0]
...
def test_ca_x_mr():
    slice_ = FrozenCube(SM.CA_X_MR).partitions[0]
    np.testing.assert_almost_equal(
...
def test_cat_x_ca_cat_x_items():
    slice_ = FrozenCube(SM.CAT_X_CA_CAT_X_ITEMS).partitions[0]
    np.testing.assert_almost_equal(
        slice_.scale_means_row, [1.34545455, 2.46938776, 2.7037037, 2.65454545]
    )
    assert slice_.scale_means_column is None
    slice_ = FrozenCube(SM.CAT_X_CA_CAT_X_ITEMS).partitions[1]
    np.testing.assert_almost_equal(
        slice_.scale_means_row, [1.41935484, 3.25663717, 3.48, 3.58536585]
    )
    assert slice_.scale_means_column is None
    slice_ = FrozenCube(SM.CAT_X_CA_CAT_X_ITEMS).partitions[2]
    np.testing.assert_almost_equal(
        slice_.scale_means_row, [1.49429038, 3.44905009, 3.59344262, 3.53630363]
    )
    assert slice_.scale_means_column is None
    slice_ = FrozenCube(SM.CAT_X_CA_CAT_X_ITEMS).partitions[3]
    np.testing.assert_almost_equal(
        slice_.scale_means_row, [1.43365696, 3.02816901, 3.37987013, 3.32107023]
    )
    assert slice_.scale_means_column is None
    slice_ = FrozenCube(SM.CAT_X_CA_CAT_X_ITEMS).partitions[4]
    np.testing.assert_almost_equal(
        slice_.scale_means_row, [1.22670025, 2.49473684, 2.79848866, 2.78987342]
    )
    assert slice_.scale_means_column is None
    slice_ = FrozenCube(SM.CAT_X_CA_CAT_X_ITEMS).partitions[5]
    np.testing.assert_almost_equal(
...
def test_cat_x_cat():
    slice_ = FrozenCube(SM.CAT_X_CAT).partitions[0]
    np.testing.assert_almost_equal(
...
def test_cat_x_mr():
    slice_ = FrozenCube(SM.CAT_X_MR).partitions[0]
    np.testing.assert_almost_equal(
...
def test_mr_x_cat():
    slice_ = FrozenCube(SM.MR_X_CAT).partitions[0]
    assert slice_.scale_means_row is None
    np.testing.assert_almost_equal(
...
def test_cat_x_cat_with_hs():
    ...
    slice_ = FrozenCube(
        CR.ECON_BLAME_X_IDEOLOGY_ROW_HS, transforms=transforms
    ).partitions[0]
    np.testing.assert_almost_equal(
...
def test_ca_x_mr_with_hs_and_pruning():
    ...
    assert slice_.scale_means_column is None
    slice_ = FrozenCube(CR.CA_X_MR_HS, transforms=transforms).partitions[1]
    np.testing.assert_almost_equal(
        slice_.scale_means_row, [2.78385708, 2.69292009, 3.11594714, np.nan]
    )
    assert slice_.scale_means_column is None
    slice_ = FrozenCube(CR.CA_X_MR_HS, transforms=transforms).partitions[2]
    np.testing.assert_almost_equal(
...
def test_cat_x_cat_pruning_and_hs():
    ...
    slice_ = FrozenCube(CR.CAT_X_CAT_PRUNING_HS, transforms=transforms).partitions[0]
    np.testing.assert_almost_equal(
...
def test_cat_x_cat_scale_means_margin():
    slice_ = FrozenCube(SM.CAT_X_CAT_SM_MARGIN).partitions[0]
    assert slice_.scale_means_row_margin == 2.6846246973365617
    assert slice_.scale_means_column_margin == 2.536319612590799


def test_cat_single_element_x_cat():
    slice_ = FrozenCube(SM.CAT_SINGLE_ELEMENT_X_CAT).partitions[0]
...
def test_means_cat_array_cat_dim_first():
    slice_ = FrozenCube(CR.PETS_ARRAY_CAT_FIRST).partitions[0]
    assert slice_.scale_means_row is None
    np.testing.assert_almost_equal(
...
def test_means_cat_array_subvar_dim_first():
    slice_ = FrozenCube(CR.PETS_ARRAY_SUBVAR_FIRST).partitions[0]
    np.testing.assert_almost_equal(
...
def test_means_cat_x_cat_arr_subvars_first():
    slice_ = FrozenCube(CR.FRUIT_X_PETS_ARRAY_SUBVARS_FIRST).partitions[0]
    np.testing.assert_almost_equal(slice_.scale_means_row, [1.71111111, 1.6, 1.65625])
    assert slice_.scale_means_column is None

    slice_ = FrozenCube(CR.FRUIT_X_PETS_ARRAY_SUBVARS_FIRST).partitions[1]
    np.testing.assert_almost_equal(
...
def test_means_cat_x_cat_arr_pets_first():
    slice_ = FrozenCube(CR.FRUIT_X_PETS_ARRAY_PETS_FIRST).partitions[0]
    ...
    slice_ = FrozenCube(CR.FRUIT_X_PETS_ARRAY_PETS_FIRST).partitions[1]
    ...
    slice_ = FrozenCube(CR.FRUIT_X_PETS_ARRAY_PETS_FIRST).partitions[2]
...
def test_means_with_null_values():
    slice_ = FrozenCube(CR.SCALE_WITH_NULL_VALUES).partitions[0]
    np.testing.assert_almost_equal(
