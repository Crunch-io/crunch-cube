...
class Assembler(object):
    ...
    def table_margin(self):
        ...
        if self._measures.columns_table_margin.is_defined:
            return self._assemble_marginal(self._measures.columns_table_margin)
        if self._measures.rows_table_margin.is_defined:
            return self._assemble_marginal(self._measures.rows_table_margin)
    ...
    def table_margin_unpruned(self):
        ...
        This value includes hidden vectors, those with either a hide transform on their
        element or that have been pruned (because their base (N) is zero). Also, it does
        not include inserted subtotals. This does not affect a scalar value but when the
        return value is an ndarray, the shape may be different than the array returned
        by `.table_margin`.

        A matrix with a multiple-response (ARRAY) dimension produces an array of
        ...
        respondents. When both dimensions are ARRAY, the return value is a 2D ndarray. A
        ...
        if self._measures.table_margin.is_defined:
            return self._measures.table_margin.value
        if self._measures.columns_table_margin.is_defined:
            return self._measures.columns_table_margin.blocks[0]
        if self._measures.rows_table_margin.is_defined:
            return self._measures.rows_table_margin.blocks[0]
