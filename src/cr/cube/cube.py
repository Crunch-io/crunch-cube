...
class CubeSet:
    ...
    @lazyproperty
    def available_measures(self) -> FrozenSet[CUBE_MEASURE]:
    ...
    @lazyproperty
    def can_show_pairwise(self) -> bool:
    ...
    @lazyproperty
    def partition_sets(self) -> Tuple[Tuple[CubePartition], ...]:
...
class Cube:
    ...
    @lazyproperty
    def dimension_types(self) -> Tuple[DT, ...]:
    ...
    def median(self) -> Optional[np.ndarray]:
        """Optional float64 ndarray of the cube_median if the measure exists."""
        if self._measures.median is None:
            return None
    ...
    @lazyproperty
    def partitions(self) -> Tuple[CubePartition, ...]:
    ...
    @lazyproperty
    def stddev(self) -> Optional[np.ndarray]:
    ...
    @lazyproperty
    def weighted_counts(self) -> Optional[np.ndarray]:
    ...
    @lazyproperty
    def weighted_valid_counts(self) -> Optional[np.ndarray]:
    ...
    @lazyproperty
    def _numeric_measure_subvariables(self) -> List[str]:
    ...
    @lazyproperty
    def _valid_idxs(self) -> Tuple[np.ndarray, ...]:
...
class _Measures:
    ...
    @lazyproperty
    def median(self) -> 'Optional["_MedianMeasure"]':
        """Optional _MedianMeasure object providing access to means values."""
        median = _MedianMeasure(
            self._cube_dict, self._all_dimensions, self._cube_idx_arg
        )
        return None if median.raw_cube_array is None else median
...
class _CovarianceMeasure(_BaseMeasure):
    ...
    @lazyproperty
    def _measure_payload(self) -> Dict:
...
class _MedianMeasure(_BaseMeasure):
    """Statistical median values from a cube-response."""
    ...
    def _flat_values(self) -> Optional[np.ndarray]:
        ...
        Median data may include missing items represented by a dict like
        {'?': -1} in the cube response. These are replaced by np.nan in the
        returned value.
        """
